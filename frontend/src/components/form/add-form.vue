              ...
              <div class="table-responsive border rounded">
                <table class="table mb-0">
                  ...
                  <tbody>
                    ...
                    <tr v-if="!searchQueries[field.key] || searchQueries[field.key].length < (field.minChars || 1)">
                    ...
                    <tr v-else-if="searchLoading[field.key]">
                      <td :colspan="(field.tableColumns ? field.tableColumns.length : 1) + 1" class="text-center py-4">
                        <span class="spinner-border spinner-border-sm text-primary me-2"></span> Searching...
                      </td>
                    </tr>
                    <!-- No results state -->
                    ...
                    <!-- Results state -->
                    <tr v-else v-for="result in searchResults[field.key]" :key="getOptionValue(result)">
                      <td v-for="col in (field.tableColumns || [{ label: 'Result', key: 'label' }])" :key="col.key">
                        {{ col.key === 'label' ? getOptionLabel(result) : (result._raw && result._raw[col.key]) }}
                      </td>
                      <td>
